...
// ✅ 공유/직링크 라우팅용
let __pendingOpenGameId = null;

function __setUrlGameParam(idOrNull){
    try{
        const url = new URL(window.location.href);
        if (idOrNull) url.searchParams.set('game', idOrNull);
...
function renderGrid() {
    ...
    const displayGames = (isAdmin ? gamesData : gamesData.filter(g => !g.isPrivate))
      .slice()
      .sort((a,b) => {
          const ab = (a.bumpAt || 0), bb = (b.bumpAt || 0);
          if (bb !== ab) return bb - ab; // ✅ 끌어올리기 최우선

          const au = _tsToMs(a.updatedAt) || 0;
          const bu = _tsToMs(b.updatedAt) || 0;
          if (bu !== au) return bu - au;

          const ac = _tsToMs(a.createdAt) || 0;
          const bc = _tsToMs(b.createdAt) || 0;
          return bc - ac;
      });
    ...
    displayGames.forEach(game => {
        ...
        // 비공개 자물쇠 아이콘
        const lockBadge = game.isPrivate
          ? `<div class="absolute top-2 left-2 bg-red-600 text-white p-1 rounded z-10"><i data-lucide="lock" class="w-3 h-3"></i></div>`
          : '';

        const badgeHtml = (game.badgeNew || game.badgeBest) ? `
          <div class="absolute top-2 right-2 z-10 flex gap-1">
            ${game.badgeBest ? `<span class="badge badge-best">BEST</span>` : ``}
            ${game.badgeNew  ? `<span class="badge badge-new">NEW</span>`  : ``}
          </div>
        ` : '';
...
window.togglePlayerMenu = () => {
    const el = document.getElementById('player-menu');
    if (!el) return;
    el.classList.toggle('hidden');
};

function renderPlayerMenuList(){
    const wrap = document.getElementById('player-menu-list');
    if (!wrap) return;

    const list = (isAdmin ? gamesData : gamesData.filter(g => !g.isPrivate))
      .slice()
      .sort((a,b) => {
          const ab = (a.bumpAt || 0), bb = (b.bumpAt || 0);
    ...
    wrap.innerHTML = list.map(g => {
        const tags = `
          ${g.badgeBest ? `<span class="badge badge-best">BEST</span>` : ``}
          ${g.badgeNew  ? `<span class="badge badge-new">NEW</span>`  : ``}
        `;
        return `
          <button class="player-menu-item" onclick="window.openPlayerFromMenu('${g.id}')">
            <div class="player-menu-item-title">${escapeHtml(g.title)}</div>
            <div class="player-menu-item-tags">${tags}</div>
          </button>
        `;
    }).join('');
...
window.openPlayer = async (id) => {
    ...
    // ✅ 모달 열 때 URL에 game=id 기록 → 공유/직링크로 재진입 가능
    __setUrlGameParam(id);
...
window.shareUrl = () => {
    try{
        ...
        // ✅ 게임 열려있으면 그 게임으로 직링크 복사
        if (currentGameId) url.searchParams.set('game', currentGameId);
...
window.closeModal = (id) => {
    ...
    // ✅ 플레이어 모달 닫을 때: URL 파라미터 제거 + iframe 정리
    if (id === 'modal-player') {
        __setUrlGameParam(null);
        currentGameId = null;
...
function initRealtimePresenceAndStats(){
    ...
    const bump = () => {
        ...
        dbRunTransaction(dbRef(rtdb, `analytics/yearly/${yearKey}/visits`), (cur) => (cur || 0) + 1);
    ...
    function drawYearlyChart(yearMap){
        ...
        const vals = years.map(y => Number(yearMap?.[y]?.visits || 0));
        ...
        const barGap = 10;
        const barW = years.length ? (innerW - barGap*(years.length-1)) / years.length : innerW;

        // bars + labels (색 지정 안 함: 기본 검정)
        years.forEach((y, i) => {
            const v = vals[i];
            ...
            ctx.fillText(y.slice(2), x + 2, H - 6);
        });

        ctx.globalAlpha = 0.7;
        ctx.font = '10px monospace';
        ctx.fillText(`max ${maxV}`, pad, 12);
        ctx.globalAlpha = 1;
    }

    onDbValue(dbRef(rtdb, `analytics/yearly`), (snap) => {
        const data = snap.val() || {};
