...
window.createRoom = async () => {
    ...
    r = Math.min(Math.max(r, 5), 100); 
    c = Math.min(Math.max(c, 5), 100);
...
function listenToRoom() {
    ...
    unsubscribeRoom = onSnapshot(roomRef, (docSnap) => {
        ...
        localGrid = JSON.parse(data.grid);
        ...
        for(let r=0; r<ROWS; r++) {
            for(let c=0; c<COLS; c++) {
                if(localGrid[r][c].state === 2) flaggedCount++;
                if(localGrid[r][c].state === 1) revealedCount++;
            }
        }
        gameState.minesLeft = gameState.config.mines - flaggedCount;
...
function createInitialGrid(config) {
    ...
    let grid = Array(ROWS).fill().map(() => Array(COLS).fill().map(() => ({
        isMine: false, state: 0, surroundingMines: 0
    })));
    
    let placed = 0;
    const maxAllowed = (ROWS * COLS) - 1;
    const targetMines = Math.min(MINES, maxAllowed);
...
function startBGM() {
    if (isMuted || bgmOscillators.length > 0) return;
    if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    if (audioCtx.state === 'suspended') audioCtx.resume();

    const notes = [220.00, 261.63, 220.00, 196.00]; 
    let noteIndex = 0;

    const playNote = () => {
        if (gameState.status !== 'playing' || isMuted) return;
        const osc = audioCtx.createOscillator();
        const gain = audioCtx.createGain();
        
        osc.type = 'square';
        osc.frequency.value = notes[noteIndex % notes.length] * (1 + (Math.floor(noteIndex/8)%2)*0.5);
